...
const MyTimetable = () => {
  ...
  const myTimetable = timetableSlots.filter(slot => slot.facultyId === user.id);
  
  const getSlot = (day: Day, period: number) => {
    return myTimetable.find(slot => slot.day === day && slot.period === period);
  };

  const totalClasses = myTimetable.length;
  const todayClasses = myTimetable.filter(slot => 
        ...
        <Card className="glass hover-lift">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-accent">
              {new Set(myTimetable.map(slot => slot.subject)).size}
            </div>
